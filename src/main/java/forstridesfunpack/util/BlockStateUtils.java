package forstridesfunpack.util;
...
public class BlockStateUtils
{
    ...
    // only works on blocks supporting IFFBlock - returns an empty set for vanilla blocks
    public static ImmutableSet<IBlockState> getBlockPresets(Block block)
    {
        if (!(block instanceof IFFBlock)) {return ImmutableSet.<IBlockState>of();}
        IBlockState defaultState = block.getDefaultState();
        if (defaultState == null) {defaultState = block.getBlockState().getBaseState();}
        return getStatesSet(defaultState, ((IFFBlock)block).getPresetProperties());
    }    
    ...
    public static IBlockState getPresetState(IBlockState state)
    {
        ...
        if (state.getBlock() instanceof IFFBlock)
        {
            IFFBlock ffBlock = (IFFBlock)state.getBlock();
            
            for (IProperty property : ffBlock.getPresetProperties())
            {
